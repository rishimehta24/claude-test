...
import { SYSTEM_PROMPT, USER_PROMPT_TEMPLATE, ALL_MODELS } from '@/lib/constants';
...
export default function PipelineBuilderTab() {
  ...
  // Auto-generate configuration name based on blocks
  const generateConfigName = (blocksToUse: PipelineBlock[]): string => {
    const processingBlocks = blocksToUse.filter(b => b.type !== 'input');
    ...
    const nameParts = processingBlocks.map(block => {
      const blockName = BLOCK_DESCRIPTIONS[block.type] || block.type;

      switch (block.type) {
        case 'llm_extraction':
        case 'llm_refinement':
        case 'llm_evaluation':
          const modelName = getModelName(block.config?.modelId);
          return modelName ? `${blockName} (${modelName})` : blockName;
        
        case 'double_layer_llm':
          const model1Name = getModelName(block.config?.modelId);
          const model2Name = getModelName(block.config?.modelId2);
          if (model1Name && model2Name) {
            return `${blockName} (${model1Name} + ${model2Name})`;
        ...
        case 'semantic_validation':
          return blockName; // Already includes "(Free)"
        
        case 'layer2_evaluator':
          return blockName; // Already includes "(Deterministic)"
  ...
  const autoGeneratedConfigName = useMemo(() => generateConfigName(blocks), [blocks]);
  ...
  const saveConfig = () => {
    ...
    const configName = autoGeneratedConfigName || `Pipeline ${Date.now()}`;
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="border-t pt-4 mb-6">
          ...
          <div className="mb-3 p-3 bg-gray-50 rounded border border-gray-200">
            <div className="text-xs text-gray-600 mb-1">Auto-generated name:</div>
            <div className="text-sm font-semibold text-gray-800">
              {autoGeneratedConfigName || 'Add blocks to generate name'}
            </div>
          </div>
          <div className="flex gap-3">
            <button
              onClick={saveConfig}
              disabled={blocks.length === 0 || (blocks.length === 1 && blocks[0].type === 'input')}
